...
class ODEBlockD(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.total_integration_time1)):
            ...
            out_ode = odeint(self.odefunc, out[i][1], self.integration_time, rtol=rtol, atol=atol)#,method='rk4')
            out.append(out_ode)
...
class Discriminator(Module):
    def __init__(self, input_dim, dis_dims, num_split, pack=1):
        ...
        self.last2 = nn.Linear(self.traj_dim*2, self.traj_dim)
        self.last3 = nn.Linear(self.traj_dim, 1)
    ...
    def forward(self, x):
        ...
        out = F.leaky_relu(self.last2(out))
        out = self.last3(out)
        return out
    ...
    def fit(self, train_data, test_data, meta_data, dataset_name, categorical_columns, ordinal_columns):
        ...
        for i in range(self.epochs):
            ...
            # if i >= 150 and i % 2 == 0:
                ...
                if len(score) == 4: # real-classification: it considers only one score; F1
                ...
                else: # real-regression(r2), macro f1, gaussian, bayesian: L_syn, L_test
                    s = score.loc[0].index.to_list()
                    for j in range(len(score)):
                    ...
                    if args.data == "news": # considers R2 score
                        avg_score = score.iloc[:, 1].mean()
                        print("epoch: {:03d} | {:.5f}".format(i, avg_score))
                        
                    elif args.data == "covtype" or "intrusion": # considers macro f1
                        avg_score = score.iloc[:, 2].mean() 
                        print("epoch: {:03d} | {:.5f}".format(i, avg_score))

                    else: # for synthetic data sets, consider 2 types of likelihoods
                        L_syn = score.iloc[:, 1].mean()
                        L_test = score.iloc[:, 2].mean()
                        print("epoch: {:03d} | {:.5f}, {:.5f}".format(i, L_syn, L_test))
                        avg_score = (L_syn + L_test)/2
