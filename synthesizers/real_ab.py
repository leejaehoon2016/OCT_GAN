...
# from evaluate_cluster import compute_scores # clustering 계산 시 사용
...
save_loc = '/home/bigdyl/jayoung/CTGAN_local/result'
...
def ODETime(num_split):
        return [torch.tensor([1 / num_split * i], dtype=torch.float32, requires_grad=False, device=device) for
            ...
            # if i >= 150 and i % 2 == 0:
                ...
                if len(score) == 4: # real-classification: it considers only one score; F1
                    ...
                    if args.data == "census" or args.data == "credit":
                        ...
                        print("epoch: {:03d} | {:.5f}, {:.5f}, {:.5f}, {:.5f}, {:.5f}".format(i, score.iloc[0, 2], score.iloc[1, 2],score.iloc[2, 2],score.iloc[3, 2],avg_score))
